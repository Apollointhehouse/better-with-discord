...
dependencies {
	...
	includeInJar(libs.jda)
	implementation(libs.discordWebhooks)
	includeInJar(libs.discordWebhooks)
}
java {
	toolchain {
...
tasks {
	...
	jar {
		...
		from(configurations.named("includeInJar").get().map { if (it.isDirectory) it else zipTree(it) })
...
configurations.matching { it.name != "includeInJar" }.configureEach { exclude(group = "org.lwjgl.lwjgl") }
